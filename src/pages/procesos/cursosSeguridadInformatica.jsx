...
export default function CoursesSeguridadInformatica() {
  ...
  const cargarCursosDelAPI = async () => {
    try {
      ...
      // Filtrar solo cursos de "Seguridad Informática" que estén activos
      const cursosDinamicos = (data.data || [])
        .filter(c => c.proceso_name === "Seguridad Informática" && c.activo)
        .map((c) => ({
          id: c.id,
          name: c.nombre,
          desc: c.descripcion,
