...
export default function CoursesSegInd() {
  ...
  const cargarCursosDelAPI = async () => {
    try {
      ...
      // Filtrar solo cursos de "Seguridad Industrial" que estén activos
      const cursosDinamicos = (data.data || [])
        .filter(c => c.proceso_name === "Seguridad Industrial" && c.activo)
        .map((c) => ({
          id: c.id,
          name: c.nombre,
          desc: c.descripcion,
