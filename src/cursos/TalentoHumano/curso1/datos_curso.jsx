export const cursoDerechosLaborales = {
  id: 1,
  nombre: "Derechos laborales de mujeres y hombres",
  descripcion:
    "Conoce los derechos fundamentales en el ámbito laboral, con enfoque de igualdad de género",
  descripcion2:
    "NOTA: Cada vez que vayas avanzando DA CLICK en Marcar Como Completado para registrar tu progreso.",
  niveles: [
    {
      id: 1,
      titulo: "Video: Legislación Laboral",
      tipo: "video",
      url: "https://www.youtube.com/watch?v=t4EHA5Q4C78", 
      completado: false,
      width: "200",
      height: "215",
      descripcion: "Video introductorio sobre los principios de los derechos laborales en el Ecuador.",
      autor: "Autor: Curriculum Minedum"
    },
    {
      id: 2,
      titulo: "Introducción a los Derechos laborales en el Ecuador",
      tipo: "video",
      url: "https://www.youtube.com/watch?v=IJzJWSkcBAA&t=4s", 
      completado: false,
      width: "200",
      height: "215",
      descripcion: "Video introductorio sobre los principios de los derechos laborales en el Ecuador.",
      autor: "Autor: CREA UE" 
    },
    {
      id: 3,
      titulo: "La base de la Justicia Laboral Igualdad y Cero Discriminación",
      tipo: "imagen",
      url: "https://ortewujmiggxjglhdmie.supabase.co/storage/v1/object/public/modulos/28/Infografia_Tecnicas_de_Aprendizaje.png",
      completado: false,
      descripcion: "Revisa el contenido de la siguiente infografía."
    },
    {
      id: 4,
      titulo: "Equidad por Ley: Lo que debes saber de la ley orgánica para la igualdad salarial (LOIS)",
      tipo: "imagen",
      url: "https://ortewujmiggxjglhdmie.supabase.co/storage/v1/object/public/modulos/28/curso1_2.png",
      completado: false,
      descripcion: "Revisa el contenido de la siguiente infografía."

    },
    {
      id: 5,
      titulo: "Contrato de Trabajo según código de trabajo en el Ecuador",
      tipo: "video",
      url: "https://www.youtube.com/watch?v=8SEF12cAL3U",
      completado: false,
      descripcion: "Video explicativo sobre el contrato de Trabajo en el Ecuador.",
      autor: "Autor: Maria Solis"
    },
    {
      id: 6,
      titulo: "Derechos Básicos de los trabajadores en el Ecuador",
      tipo: "video",
      url: "https://www.youtube.com/watch?v=2MrEKGvsEdg&t=479s",
      completado: false,
      descripcion: "Video informativo sobre los derechos básicos de los trabajadores en el Ecuador",
      autor: "Autor: In Solidum Abogados"
    },
    {
      id: 7,
      titulo: "Derechos Laborales y Ley Orgánica para la Igualdad Salarial",
      tipo: "pdf",
      url: "/pdf/Derechos_Laborales_de_Hombres_MujeresV2.pdf",
      completado: false,
      descripcion: "Lee el siguiente documento para entender los derechos laborales y la ley de igualdad salarial."
    },
    {
      id: 8,
      titulo: "Derechos laborales específicos de las mujeres",
      tipo:"video",
      url: "https://www.youtube.com/watch?v=o9mvHtRNQyA",
      completado: false,
      descripcion: "Video explicativo sobre los derechos laborales específicos de las mujeres.",
      autor: "FES Ecuador"
    },
    {
      id: 9,
      titulo:"¿Por qué los Hombres ganan más que las Mujeres?", 
      tipo: "video",
      url: "https://www.youtube.com/watch?v=iOAirOHr-Rs",
      completado: false,
      width: "200",
      height: "215",
      descripcion: "Video explicativo sobre la inequidad salarial entre hombres y mujeres en el Ecuador.",
      autor: "Autor: FRANCE 24 en Español"
    },
    {
      id: 10,
      titulo: "PODCAST: Derechos laborales de Mujeres y Hombres en el Ecuador",
      tipo: "video",
      url: "https://www.youtube.com/watch?v=X9wxgZtIFvw",
      completado: false,
      descripcion: "Podcast informativo sobre los derechos laborales de mujeres y hombres en el Ecuador.",
      autor: "Autor: Marcelo Valverde Guevara"
    }
  ],
};
