export const cursoErradicacionViolencia = {
  id: 3,
  nombre: "Erradicación de violencia y no discriminación en una empresa o centro de trabajo",
  descripcion:
    "Conoce las causas y como erradicar la violencia y discriminación en una empresa o centro de trabajo.",
  descripcion2: 
    "NOTA: Cada vez que vayas avanzando DA CLICK en Marcar Como Completado para registrar tu progreso.",
  niveles: [
    {
        id: 301,
        titulo: "Acoso Laboral y Violencia en el Ecuador",
        tipo: "video",
        url: "https://res.cloudinary.com/dlupri3mm/video/upload/v1765831246/Acoso_Laboral_y_Violencia_en_Ecuador_g1bedx.mp4", 
        completado: false,
        width: "200",
        height: "215",
        descripcion: "Video sobre la violencia y discriminación en el trabajo.",
        autor: "Autor: Prevención Ecuador"
    },
    {
        id: 302,
        titulo: "Cero Tolerancia a La Violencia Laboral",
        tipo: "imagen",
        url: "https://res.cloudinary.com/dlupri3mm/image/upload/v1787084919/CERO_TOLERANCIA_A_LA_VIOLENCIA_LABORAL_e7z3l2.png",
        completado: false,
        descripcion: "Revisa el contenido de la siguiente infografía."
    },
    {
        id: 303,
        titulo: "No discriminación - Inclusión",
        tipo: "imagen",
        url: "https://res.cloudinary.com/dlupri3mm/image/upload/v1787084935/NO_DISCRIMINACI%C3%93N_-_INCLUSI%C3%93N_c1uppk.png",
        completado: false,
        descripcion: "Revisa el contenido de la siguiente infografía."

    },
    {
        id: 304,
        titulo: "Discriminación Laboral",
        tipo: "video",
        url: "https://res.cloudinary.com/dlupri3mm/video/upload/v1787146236/Brechas_laborales_que_a%C3%BAn_enfrentan_las_mujeres_en_Ecuador_idcqo7.mp4",
        completado: false,
        descripcion: "Revisa el siguiente reportaje sobre las Brechas laborales entre hombres y mujeres en el Ecuador.",
        autor: "Autor: Teleamazonas"
    },
    {
        id: 305,
        titulo: "Canal de Denuncias - Protocolos",
        tipo: "imagen",
        url: "https://res.cloudinary.com/dlupri3mm/image/upload/v1787084951/CANAL_DE_DENUNCIAS_-_PROTOCOLOS_ryfqw2.png",
        completado: false,
        descripcion: "Revisa el contenido de la siguiente infografía."
    },
    {
        id: 306,
        titulo: "PODCAST: Violencia en el trabajo: ¿Cómo Detectarla y qué hacer al respecto?",
        tipo: "video",
        url: "https://www.youtube.com/watch?v=6B8d8SSzasU",
        completado: false,
        width: 200,
        height: 215,
        descripcion: "Revisa el siguiente podcast informativo."
    },
    {
        id: 307,
        titulo: "¿Cómo erradicar la violencia y discriminación en una empresa o centros de trabajo?",
        tipo: "pdf",
        url: "/pdf/Violencia_Discriminacion_en_el_trabajoV2.pdf",
        completado: false,
        descripcion: "Lee el siguiente documento para entender la relación entre igualdad de género."
    },
    {
        id: 308,
        titulo: "Cómo prevenir la Violencia Laboral",
        tipo: "video",
        url: "https://res.cloudinary.com/dlupri3mm/video/upload/v1765831287/Como_prevenir_la_violencia_laboral_u8xnny.mp4",
        completado: false,
        descripcion: "En este video informativo aprenderás cómo prevenir la violencia laboral."
    },
    {
      id: 309,
        titulo: "La violencia en el trabajo, Discriminación Laboral",
        tipo: "video",
        url: "https://res.cloudinary.com/dlupri3mm/video/upload/v1765831449/La_violencia_en_el_trabajo_Discriminacion_en_el_trabajo_usfolm.mp4",
        completado: false,
        descripcion: "En este video informativo aprenderás como actuar ante la violencia en el trabajo y la discriminación laboral con esta dramatización."
    }
  ],
};
